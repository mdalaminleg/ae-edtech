...
async function handleStudent(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/payment/submit') {
    ...
    if (!course_id || !payMethod || !trx_id || !phone) return err('All payment fields required (course_id, method, trx_id, phone)');
...
async function handleAdmin(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/admin/courses') {
    ...
    await db.prepare(`INSERT INTO courses (title, description, subject_area, difficulty, thumbnail_url, thumbnail_icon, badge, price, tree, resources, is_featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
