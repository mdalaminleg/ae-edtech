...
async function ensureTables(db) {
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS courses (
    ...
    thumbnail_icon TEXT DEFAULT 'book-open',
    badge TEXT DEFAULT '',
  ...
  if (method === 'POST' && path === '/auth/login') {
    ...
    return json({
      ...
      user: {
        ...
        phone: user.phone, institution: user.institution,
...
async function handleStudent(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/payment/submit') {
    ...
    if (spamCount && spamCount.c >= 3) return err('Account flagged for suspicious activity', 403);
    ...
    await db.prepare('INSERT INTO enrollments (user_id, course_id, payment_method, trx_id, sender_phone, status) VALUES (?, ?, ?, ?, ?, ?)')
...
async function handleAdmin(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/admin/courses') {
    ...
    await db.prepare(`INSERT INTO courses (title, description, subject_area, difficulty, thumbnail_url, thumbnail_icon, badge, price, tree, resources, is_featured)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(title, description || '', subject_area || '', difficulty || 'All Levels', thumbnail_url || '', thumbnail_icon || 'book-open', badge || '', price || 0, JSON.stringify(tree || []), JSON.stringify(resources || []), is_featured || 0).run();
